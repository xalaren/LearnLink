...
{
    public class UsersService(IUserRepository userRepository, ICredentialsRepository credentialsRepository, IUnitOfWork unitOfWork, IEncryptionService encryptionService, ILogger logger)
    {
        ...
        public async Task<Result> RegisterAsync(RegisterRequest request, string plainPassword)
        {
            try
            {
                ...
                if (exists) return Result.Conflict($"User with nickname '{request.Nickname}' already exists");
